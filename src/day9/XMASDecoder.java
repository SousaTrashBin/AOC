...
public class XMASDecoder {
    ...
    private boolean isValid(long number) {
        ...
        for (long num1 : preamble) {
            ...
            if (set.contains(num2) && num1 != num2) return true;
        }
        return false;
    }
    ...
    public long findFirstInvalidNumber() {
        for (long number : numbers) {
            ...
            if (preamble.size() == PREAMBLE_SIZE) preamble.poll();
    ...
    public List<Long> findContiguousSet() {
        for (int start = 0; start < numbers.size(); start++) {
            ...
            for (int end = start + 1; end < numbers.size(); end++) {
                sum += numbers.get(end);
                if (sum == finalValue) return numbers.subList(start, end + 1);
    ...
    public long getEncryptionWeakness() {
        ...
        long first = firstAndLast
            .stream()
            .min(Long::compareTo)
            .orElseGet(() -> (long) -1);
        long last = firstAndLast
            .stream()
            .max(Long::compareTo)
            .orElse(0L);
        return first + last;
